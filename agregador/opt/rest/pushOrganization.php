...
try { 
	$directorio->openByField ("token", $_GET ["token"]); }
catch (exception $e) { 
	die ("token error");
}
...
try { 
	$organizacion->openByField ("cif",  $xml->legalID); 
	echo ("cif found\n");
}catch (exception $e) { 
	$organizacion->cif = (string) $xml->legalID; 
	$organizacion->directorio_id = $directorio->id; 
}

echo($directorio->id . " xxxxxxx " . $organizacion->directorio_id . "\n");

if ($organizacion->directorio_id != $directorio->id) {
	echo("Problema de directorio.\n");
	die ("directory error");
}

echo("Nombre: " . $xml->name . "\n");	
$organizacion->nombre = (string) $xml->name;
echo("Descripcion: " . $xml->description . "\n");	
$organizacion->descripcion = (string) $xml->description;

if ($xml->web) {
	$organizacion->web = (string) $xml->web;
	echo("Web: " . $xml->web . "\n");	
}
$organizacion->direccion = (string) $xml->HQstreet;
echo("Dirección: " . $xml->HQstreet . "\n");	
$organizacion->localidad = (string) $xml->HQlocality;
echo("Localidad: " . $xml->HQlocality . "\n");	
$organizacion->formaJuridica_id = (string) $xml->organizationType ["code"];
echo("Forma Jurídica: " . $xml->organizationType ["code"] . "\n");	
...
foreach ($organizacion->getChildren ("capacidadOfertaOrganizacion") as $oferta) 
	$oferta->delete ();
...
foreach ($organizacion->getChildren ("evento") as $evento) 
	$evento->delete ();
