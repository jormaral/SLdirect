 ...
 */
@Controller
@RequestMapping("/fj/{formaJuridicaId}/edit")
@SessionAttributes(types = FormaJuridica.class)
public class EditFormaJuridicaForm {
	private final static Logger logger = LoggerFactory.getLogger(EditFormaJuridicaForm.class);
	...
	public String setupForm(@PathVariable("formaJuridicaId") int formaJuridicaId, Model model) {
		FormaJuridica formaJuridica;
		try {
			formaJuridica = formaJuridicaService.findById(formaJuridicaId);
	...
	public String processSubmit(@ModelAttribute FormaJuridica formaJuridica, BindingResult result, SessionStatus status) {
		if (logger.isTraceEnabled())
			logger.trace("Validating Forma Juridica: " + formaJuridica);
