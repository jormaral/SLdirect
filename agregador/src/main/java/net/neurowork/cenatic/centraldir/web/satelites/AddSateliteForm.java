 ...
 */
@Controller
@RequestMapping("/satelites/new")
@SessionAttributes(types = Satelite.class)
public class AddSateliteForm {
	private final static Logger logger = LoggerFactory.getLogger(AddSateliteForm.class);
	...
	public String updateSubmit(@ModelAttribute Satelite satelite, BindingResult result, SessionStatus status) {
		new SateliteValidator().validate(satelite, result);
		if (!result.hasErrors()) {
			try {
				this.sateliteService.saveSatelite(satelite);
				status.setComplete();
				return "redirect:/satelite/" + satelite.getId();
			} catch (ServiceException e) {
				logger.error(e.getMessage());
			}
		}
