 ...
 */
@Controller
@RequestMapping("/provincias/new")
@SessionAttributes(types = Provincia.class)
public class AddProvinciaForm {
	private final static Logger logger = LoggerFactory.getLogger(AddProvinciaForm.class);
	...
	public String setupForm(Model model) {
		if(logger.isTraceEnabled())
			logger.trace("New Provincia Form");
		model.addAttribute(new Provincia());
	...
	public String processSubmit(@ModelAttribute Provincia provincia, BindingResult result, SessionStatus status) {
		if(logger.isTraceEnabled())
			logger.trace("Validating Provincia: " + provincia);
		...
		if (!result.hasErrors()) {
			try {
				if(logger.isTraceEnabled())
					logger.trace("Provincia: " + provincia + " OK. Saving");
