...
";
		...
		$data .= "  <capacities>
";
		foreach ($capacidades as $capacidad) $data .= "      <capacity capacity_code=\"" . $capacidad->capacidad_id->id . "\" sector_code=\"" . $capacidad->sector_id->id . "\" resources=\"" . $capacidad->recursos . "\" score=\"" . $capacidad->puntuacion . "\" billingProportion=\"" . $capacidad->porcentajeFacturacion . "\" />
";
		$data .= "  </capacities>
";
		...
		$data .= "  <demands>
"; 
		foreach ($demandas as $demanda) $data .= "    <demand capacity_code=\"" . $demanda->capacidad_id->id . "\" sector_code=\""  .$demanda->sector_id->id . "\" />
";
		$data .= "  </demands>
";
...
");
    fputs ($socket, $data);
		//$response = ""; while (!feof ($socket)) $response .= fgets ($socket, 1024); die ("<pre>" . htmlspecialchars ($data . "
" . $response) . "</pre>");
	}
}
