...
if (array_key_exists("nombre",$_REQUEST)){
	if(!isset($_REQUEST["certificacionCenatic"]))
		$organizacion->certificacionCenatic = 0;
	if(!isset($_REQUEST["actividadesImasD"]))
		$organizacion->actividadesImasD = 0;
	if ($_REQUEST["web"]{0} != "h" && $_REQUEST["web"]{4} != ":") $_REQUEST["web"] = "http://".$_REQUEST["web"];
	...
	foreach ($_REQUEST["certificacionesCalidad"] as $certificacion) {
		if($certificacion != '')
			$certificaciones[] = $certificacion;
	}
	$organizacion->certificacionesCalidad = base64_encode(serialize($certificaciones));
