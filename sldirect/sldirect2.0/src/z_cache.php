...
$result = simplexml_load_file ($url . "getOrganizationClassList.php");
foreach ($result->class as $row) {
  $item = new claseEmpresa ();
  $item->open ($row ["code"]);
  if ($item->id == null) $database->query ("INSERT into clases_empresas (id, nombre) values (" . $row ["code"] . ", \"" . mysql_escape_string ($row) . "\");");
}
